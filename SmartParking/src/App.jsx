  ...
  const handleSensorUpdate = (sensorSaysOccupied) => {
    setIsOccupied(prev => {
      if (prev !== sensorSaysOccupied) {
        if (sensorSaysOccupied) { setEntryTime(Date.now()); setPaymentSuccess(false); } else { setShowPayment(true); }
      }
      ...
      <div className="bg-slate-950/50 backdrop-blur-md border-b border-white/10 p-4 sticky top-0 z-20">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-blue-600 p-2 rounded-lg"><Car className="h-6 w-6 text-white" /></div>
            <h1 className="font-bold text-xl tracking-tight text-white">SmartPark View</h1>
          </div>
          <button onClick={isConnected ? disconnectSerial : connectToSerial}
            className={`flex items-center gap-2 text-sm font-bold px-4 py-2 rounded-lg transition-all border ${isConnected ? "bg-green-600/20 text-green-400 border-green-500/50 hover:bg-green-600/30" : "bg-slate-800 text-slate-300 border-slate-600 hover:bg-slate-700 hover:border-slate-500"}`}>
            {isConnected ? <CheckCircle className="h-4 w-4" /> : <Usb className="h-4 w-4" />}
      ...
      <div className="max-w-5xl mx-auto p-4 space-y-6 mt-4">
        {errorMsg && (
          <div className="bg-red-950/50 border border-red-800 rounded-lg p-3 flex items-start gap-3 text-red-200">
            ...
            <div className="flex-1"><h3 className="text-sm font-bold">Connection Issue</h3><p className="text-xs opacity-80 mt-1">{errorMsg}</p></div>
            <button onClick={() => setErrorMsg(null)} className="text-red-400 hover:text-red-200"><XCircle className="h-4 w-4" /></button>
          </div>
        )}
        ...
        <div className="relative w-full max-w-4xl mx-auto rounded-xl overflow-hidden shadow-2xl border-4 border-slate-700/50 bg-slate-800">
          {/* BACKGROUND IMAGE - Using the imported variable ensures it loads */}
          <img 
            src={parkingBg} 
        ...
        {/* --- LIVE SPOT DETAILS PANEL --- */}
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className={`col-span-1 md:col-span-2 rounded-xl border p-4 flex items-center gap-4 transition-colors duration-500 ${isOccupied ? "bg-red-950/30 border-red-900/50" : "bg-green-950/30 border-green-900/50"}`}>
                <div className={`h-16 w-16 rounded-full flex items-center justify-center border-2 ${isOccupied ? "bg-red-900/50 border-red-500 text-red-100" : "bg-green-900/50 border-green-500 text-green-100"}`}>
                    {isOccupied ? <Car className="h-8 w-8" /> : <span className="text-2xl font-bold">P</span>}
                </div>
                <div>
                    <div className="text-sm font-bold opacity-60 mb-1 flex items-center gap-2">
                        LIVE SENSOR STATUS (Spot #101)
                        {isOccupied && <span className="inline-block w-2 h-2 bg-red-500 rounded-full animate-pulse"/>}
                    </div>
                    <div className={`text-2xl font-black tracking-wide ${isOccupied ? "text-red-100" : "text-green-100"}`}>
            ...
            <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4 flex flex-col justify-center">
                <div className="flex justify-between items-center mb-2">
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1"><Clock className="h-3 w-3" /> Duration</div>
                    <div className={`font-mono text-xl font-bold ${isOccupied ? "text-white" : "text-slate-500"}`}>
                        {isOccupied ? formatTime(elapsedTime) : "--:--:--"}
                    </div>
                </div>
                <div className="border-t border-slate-700/50 my-2"></div>
                <div className="flex justify-between items-center">
                     <div className="text-xs font-bold text-blue-400 uppercase tracking-wider flex items-center gap-1"><DollarSign className="h-3 w-3" /> Current Fee</div>
                     <div className={`font-mono text-2xl font-black ${isOccupied ? "text-blue-400" : "text-slate-500"}`}>
                        ${isOccupied ? fee.toFixed(2) : "0.00"}
                    </div>
                </div>
                 <div className="text-[10px] text-center text-slate-500 mt-2">Rate: ${HOURLY_RATE.toFixed(2)}/hr</div>
            </div>
        </div>
        ...
        {!isConnected && (
          <div className="bg-yellow-950/30 border border-yellow-700/50 rounded-lg p-4 flex items-center justify-between max-w-md mx-auto text-yellow-200">
            <div className="flex items-center gap-3">
                ...
                <div><h3 className="text-sm font-bold">Simulation Mode</h3><p className="text-xs opacity-80">Test the live spot (#101) without hardware.</p></div>
            </div>
            <button onClick={handleManualSimulate} className="text-xs bg-yellow-600 hover:bg-yellow-500 text-white font-bold py-2 px-4 rounded-lg transition-colors">Toggle #101</button>
          </div>
        )}
      </div>
          ...
          <div className="bg-slate-900 w-full max-w-sm rounded-2xl shadow-2xl border border-slate-700 overflow-hidden">
            {!paymentSuccess ? (
              <div className="p-6 text-white">
                <div className="flex justify-between items-start mb-6"><div><h3 className="text-xl font-bold">Payment Due</h3><p className="text-slate-400 text-sm">Spot #101</p></div><div className="bg-blue-900/50 text-blue-300 border border-blue-700/50 font-bold px-3 py-1 rounded-lg text-lg">${fee.toFixed(2)}</div></div>
                <button onClick={() => { setTimeout(() => { setPaymentSuccess(true); setTimeout(() => { setShowPayment(false); setIsOccupied(false); setEntryTime(null); setElapsedTime(0); setFee(0); }, 2500); }, 1000); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-colors"><CreditCard className="h-5 w-5" />Pay Now</button>
              </div>
            ) : (
              <div className="p-10 flex flex-col items-center bg-green-950/30 text-center text-white"><div className="w-20 h-20 bg-green-900/50 rounded-full flex items-center justify-center mb-4 text-green-400 ring-8 ring-green-900/20 border border-green-500/50"><CheckCircle className="h-10 w-10" /></div><h3 className="text-2xl font-bold mb-2">Paid!</h3></div>
            )}
          </div>
        </div>
      )}
